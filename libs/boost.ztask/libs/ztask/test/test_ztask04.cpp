...
int main() {
  ...
  // create handlers
  handle<const long&> h1;
  handle<const long&> h2;
  ...
  {
    // create task
    task<const long&> t1(boost::bind(threadsafe_fibonacci2, 20, boost::ref(r1), 1));
    task<const long&> t2(boost::bind(threadsafe_fibonacci2, 5, boost::ref(r2), 2));
